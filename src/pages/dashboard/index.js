...
export async function getServerSideProps(ctx) {
  ...
  if (session) {
    try {
      ...
      const userLoggings = await Logging.find({ rbUserId: 6237218 })
        .sort({ rbCommentId: -1 })
        .limit(10)
        .lean();
      ...
      const projects = await Project.find({
        rbProjectId: { $in: projectIds },
      }).lean();

      const loggingsWithTasksAndProjects = userLoggings.map((logging) => {
        const task = userTasks.find(
          (task) => task.rbTaskId === logging.rbTaskId
        );
        const project = projects.find(
          (project) => project.rbProjectId === task.rbProjectId
        );
      ...
      return {
        props: {
          ...
          loggingsWithTasksAndProjects: JSON.parse(
            JSON.stringify(loggingsWithTasksAndProjects)
          ),
...
export default function Home({
  ...
  return (
    <>
      ...
      <Dashboard
        projects={projects}
        userData={loggingsWithTasksAndProjects}
